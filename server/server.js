...
const uri = "mongodb+srv://djangoadmin:zpxHwEVKtbGsYIP6@cluster0.0ube0.mongodb.net/Login_system?retryWrites=true&w=majority";
const app = express();
app.use(cors());
...
async function getEmpAttendance(emp_data) {
  ...
  for (let date of dates_arr) {
    let pr = await getEmpAttendanceOnDate(date.getDate(), date.getMonth(), date.getFullYear(), emp_id.valueOf());
...
app.get('/change-employee-attendance', (req, res) => {
  ...
  if (emp_id === null || action === null) {
    ...
    if (action === 'true') { // add in todays list
...
app.get('/get-employee/:id', async (req, res) => {
  ...
  if (emp_data) {
    let [year, month, date] = getTodayDate();
    let isPresent = await getEmpAttendanceOnDate(date, month, year, emp_data._id.valueOf());
    let emp_attendance = await getEmpAttendance(emp_data);
    res.status(200).send({ 'data': emp_data, 'present': isPresent, 'attendance_data': emp_attendance })
  }
  else {
